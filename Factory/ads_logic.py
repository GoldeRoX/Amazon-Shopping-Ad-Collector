...
class AdHandler(object):
    ...
    def execute_ad_1(self, driver, ad_text_filter: [str], session_id: int) -> None:
        try:
            ads_list = self.collect_ads_1(driver)
            for ad in ads_list:
                MyDriver.save_cropped_scr(driver, ad)
    ...
    def execute_ad_2(self, driver, ad_text_filter: [str], session_id: int) -> None:
        try:
            ads_webelements = self.get_webelements_ads_2(driver)
    ...
    def collect_ads_1(self, driver) -> [Ad]:
        ads = []

        webelements = self.get_webelements_ads_1(driver)
    ...
    def collect_ads_2(self, driver) -> [Ad]:
        ads = []
        ads_webelements = self.get_webelements_ads_2(driver)
    ...
    def execute_ad_4(self, driver, ad_text_filter: [str], session_id: int):
        try:
            ads_webelements = self.get_webelements_ads_4(driver)
    ...
    def execute_ad_5(self, driver, ad_text_filter: [str], session_id: int):
        try:
            ads_webelements = self.get_webelements_ads_5(driver)
